...
But, *in addition* to what we did in class, add a column to this dataset called `Matrix` that has the value "WB" for every row. (This way when we eventually combine all three datasets, we'll know which values came from which dataset.)
...
-   The following columns renamed: `thcoh = x11_oh_th`; `thcv = thc_v`
...
-   add a column to this dataset called `Matrix` that has the value "OF" for every row
...
-   The following columns renamed: `thc = thc_pg_pad`
